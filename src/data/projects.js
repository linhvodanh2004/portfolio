...
export const projects = [
  {
    id: "itseek",
    title: "ITSeek — Nền tảng tuyển dụng IT tích hợp AI",
    ...
    scope: {
      ...
      en: "Application + CV modules, Gemini AI integration and the whole testing layer",
    },
    desc: {
      ...
      en: "Capstone-scale system: Spring Boot 3.5 / Java 21 with ~57 REST controllers, ~55 JPA entities, realtime over SSE + Socket.IO, and Gemini AI threaded through the CV → matching → mock-interview flow.",
    },
    highlights: [
      {
      ...
      },
      {
        ...
        en: "Semantic search & job matching on vector embeddings stored in PostgreSQL + pgvector, combined with a weighted scorer (JobMatchScorer, CriteriaWeightConfig) instead of relying on the LLM alone.",
      },
      {
        ...
        en: "End-to-end CV builder: autosaving editor with version history/rollback, public share tokens, and a render pipeline turning one data source into HTML / PDF (Flying Saucer) / DOCX (Apache POI).",
      },
      {
        ...
        en: "Two-sided Application module: seeker side (submit, eligibility check, withdraw) and recruiter side (status transitions, hold/unhold, notes, external application from a CV) — every transition publishes an event to the notification hub.",
      },
      {
        ...
        en: "Layered testing: JUnit 5 + Mockito unit tests for scorers/utils, ~45 MockMvc controller integration tests, Playwright e2e per business flow, and a k6 load-test suite measuring p95 against each NFR.",
      },
    ],
    stack: [
  ...
  {
    id: "slotify",
    title: "Slotify — Hệ thống đặt vé xem phim",
    cats: ["Backend", "Frontend"],
    ...
    desc: {
      ...
      en: "A .NET 8 backend split into Presentation / BusinessLogic / DataAccess layers with a React 19 + TypeScript frontend. I owned the authentication flow and the admin area end to end.",
    },
    highlights: [
      {
      ...
      },
      {
        ...
        en: "Added Google OAuth (Google.Apis.Auth), BCrypt password hashing and a forgot/reset-password email flow — with every 401/403 response normalised into a single ApiResponse wrapper.",
      },
      {
      ...
      },
      {
        ...
        en: "Role-based route guards on the frontend kept in sync with JWT authorisation on the backend — never protecting the admin area from only one side.",
      },
      {
        ...
        en: "Infrastructure: EF Core + SQL Server, AutoMapper, Cloudinary for posters, a BackgroundService expiring stale bookings, a GlobalExceptionHandler and Swagger across the API.",
      },
    ],
    stack: [
  ...
  {
    id: "waste-to-worth",
    title: "Waste To Worth — E-commerce bản đồ gỗ tái chế",
    cats: ["Backend", "Frontend"],
    ...
    highlights: [
      {
        ...
        en: "Module-per-domain NestJS architecture (auth, product, cart, order, category, wood-type, admin-dashboard…) with shared guards/decorators — adding a domain never means touching existing code.",
      },
      {
        ...
        en: "Authorisation via a JWT strategy plus RolesGuard and a `@Roles()` decorator; Google sign-in through Passport google-oauth20 with a callback flow handing the token back to the frontend.",
      },
      {
        ...
        en: "A complete relational Prisma schema (User, Product, WoodType, Category, CartItem, Order/OrderItem, Review) with order-status and payment-method enums — type-safe from the database up to the controller.",
      },
      {
        ...
        en: "DTO validation with class-validator/class-transformer, auto-generated Swagger docs, image upload through Multer + Cloudinary, transactional email via Nodemailer/Brevo, and scheduled jobs with @nestjs/schedule.",
      },
      {
  ...
  {
    id: "staybook",
    title: "StayBook — App đặt phòng theo giờ",
    cats: ["Mobile"],
    ...
    highlights: [
      {
        ...
        en: "Clear layering inside Flutter: models (fromMap/toMap), DTOs for every write, and services wrapping all Firestore queries — screens never touch Firestore directly.",
      },
      {
        ...
        en: "Radius search with geohashes (geoflutterfire_plus) plus geolocator/geocoding — spatial queries on a Firestore that has no native support for them.",
      },
      {
      ...
      },
      {
        ...
        en: "Role-driven navigation and permissions in a single app: User (explore, book, vouchers), Host (rooms, calendar, per-day pricing, revenue) and Admin (approvals, users, reports).",
      },
      {
        ...
        en: "Direct Cloudinary uploads over client-side SHA-1 signed HTTP requests, realtime chat on Firestore streams, fl_chart revenue charts and a table_calendar booking calendar.",
      },
    ],
    stack: [
  ...
  {
    id: "fsa-online-shop",
    title: "GOS Shop — E-commerce Spring Boot MVC",
    cats: ["Backend", "Frontend"],
    ...
    desc: {
      ...
      en: "A Spring Boot MVC + Thymeleaf storefront built solo — from the security layer and VNPay payments through to a Gemini chatbot with conversational memory.",
    },
    highlights: [
      {
        ...
        en: "A Gemini chatbot with a hand-rolled memory layer: history in the database for signed-in users and in the session for guests, a bounded context window, and summarisation past a threshold to keep tokens in check.",
      },
      {
        ...
        en: "Multi-path Spring Security: form login plus Google OAuth2 through a CustomOAuth2UserService, remember-me, role-based authorisation, and a one-session-per-account limit backed by a ForceLogoutFilter.",
      },
      {
      ...
      },
      {
        ...
        en: "Centralised sessions with Spring Session JDBC (cart and session survive a restart), verification/reset emails through Spring Mail, and SEO-friendly slug URLs for products.",
      },
      {
        ...
        en: "Interface/implementation split across the whole service layer, dedicated DTOs and mappers, an admin dashboard with monthly revenue reporting, and CRUD for products/categories/orders/users across 36 Thymeleaf templates.",
      },
    ],
    stack: [
